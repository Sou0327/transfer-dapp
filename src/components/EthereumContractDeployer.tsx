...
interface EthereumContractDeployerProps {
  className?: string
}

/**
 * EthereumネットワークでのERC-20カスタムコントラクト作成・管理コンポーネント
 */
export const EthereumContractDeployer: React.FC<EthereumContractDeployerProps> = ({ className = '' }) => {
  ...
  // Ethereum Provider初期化
  useEffect(() => {
    if (multiWallet.metamask.isConnected) {
      const provider = createProvider()
      if (provider) {
        ethereumContractService.setProvider(provider)
      }
    }
  ...
  useEffect(() => {
    const saved = localStorage.getItem('deployed-erc20-contracts')
    if (saved) {
      try {
  ...
  const saveDeployedContracts = useCallback((contracts: DeployedContract[]) => {
    localStorage.setItem('deployed-erc20-contracts', JSON.stringify(contracts))
  ...
  const handleDeploy = useCallback(async () => {
    ...
    if (!multiWallet.metamask.isConnected) {
      toast.error('ウォレット未接続', 'MetaMaskを接続してください')
      return
    }
    ...
    try {
      toast.info('デプロイ開始', 'ERC-20トークンをEthereumネットワークにデプロイしています...')

      // Solidityコードを生成
      const sourceCode = ethereumContractService.getBasicTokenTemplate(
        tokenName.trim(),
      ...
      // コンパイル
      const compileResult = await ethereumContractService.compileSolidity(sourceCode)
      if (!compileResult.success || !compileResult.abi) {
        throw new Error(compileResult.error || 'コンパイルに失敗しました')
      }

      // デプロイ
      const deployResult = await ethereumContractService.deployContract(compileResult.abi, sourceCode)
      ...
      toast.success(
        'デプロイ完了！', 
        `${newContract.symbol}トークンがEthereumネットワークに正常にデプロイされました\n${deployResult.address}`
      )
  ...
  }, [validateDeployForm, multiWallet.metamask.isConnected, tokenName, tokenSymbol, totalSupply, deployedContracts, saveDeployedContracts, toast])

  // 残高取得
  const updateBalance = useCallback(async () => {
    if (!selectedContract || !multiWallet.metamask.account) return

    try {
      const balance = await ethereumContractService.getTokenBalance(
  ...
  }, [selectedContract, multiWallet.metamask.account])
  ...
  const handleTransfer = useCallback(async () => {
    ...
    if (!ethereumContractService.isValidAddress(transferTo.trim())) {
      toast.error('入力エラー', '有効なEthereumアドレスを入力してください')
      return
    }
    ...
    try {
      ...
      const txHash = await ethereumContractService.transferCustomToken(
        selectedContract.address,
        selectedContract.abi,
        transferTo.trim(),
        transferAmount.trim()
  ...
  return (
    <div className={`ethereum-contract-deployer ${className}`}>
      <div className="contract-deployer-header">
        <h3>🚀 ERC-20トークン作成（Ethereum）</h3>
        <p>オリジナルのERC-20トークンを作成してEthereumネットワークにデプロイできます</p>
      </div>

      {!multiWallet.metamask.isConnected ? (
        <div className="wallet-connection-required">
          <div className="warning-icon">⚠️</div>
          <h4>MetaMask接続が必要です</h4>
          <p>カスタムコントラクトを作成するにはMetaMaskウォレットを接続してください</p>
        </div>
      ) : (
        <div className="contract-deployer-content">
          {/* デプロイフォーム */}
          <div className="deploy-section">
            <h4>📝 新しいERC-20トークンを作成</h4>
            ...
            <div className="form-group">
              ...
              <input
                ...
                disabled={isDeploying}
              />
              <small>デプロイ後にあなたのウォレットに全量が送られます（18桁小数）</small>
            </div>
            ...
            >
              {isDeploying ? '⏳ デプロイ中...' : '🚀 Ethereumにデプロイ'}
            </button>
          </div>
              ...
              <div className="transfer-section">
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    placeholder="0xから始まるEthereumアドレス"
...
export default EthereumContractDeployer