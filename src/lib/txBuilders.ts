...
abstract class BaseTxBuilder {
  ...
  protected buildTxBody(inputs: any, outputs: any, currentSlot: any, fee?: bigint): any {
    ...
    // 🎯 EXPERT RECOMMENDED: 署名前TTL確定ロジック
    let ttl;
    if (this.config.ttlSlot && this.config.ttlSlot > 0) {
      console.log('🔧 Request TTL provided:', this.config.ttlSlot);
      
      // 🚨 CSL BigNum overflow prevention (2^53 threshold)
      const TTL_OVERFLOW_THRESHOLD = 100000000; // ~100M slots
      
      if (this.config.ttlSlot > TTL_OVERFLOW_THRESHOLD) {
        console.log('🔧 TTL overflow detected, using safe alternative');
        const safeOffset = 7200; // 2 hours
        ttl = currentSlot.checked_add(CSL.BigNum.from_str(safeOffset.toString()));
        console.log('✅ SAFE TTL (署名前確定):', ttl.to_str());
        console.log('⚠️ Original TTL was too large:', this.config.ttlSlot);
      } else {
        // Request TTL is safe, use as-is
      ...
      console.log('✅ CALCULATED TTL (署名前確定):', ttl.to_str());
    }

    console.log('📅 Final TTL (署名前確定完了):', ttl.to_str());
    
    // 🚨 CRITICAL: TTL is now FINAL - 署名後は一切変更しない
