...
const App: React.FC = () => {
  ...
  const displayError = useMemo(() => {
    return error || connectionError || utxoError;
  ...
  const debugInfo = useMemo(() => ({
    ...
    utxoError,
    activeView
  }), [isConnected, networkId, utxos.length, utxosLoading, connectionError, utxoError, activeView]);

  React.useEffect(() => {
    console.log('🔍 App.tsx state:', debugInfo);
  }, [debugInfo]);
  ...
  const connectionInfo = useMemo(() => {
    ...
    return {
      ...
      activeViewLabel: activeView === 'dashboard' ? 'ダッシュボード' : '送金'
    };
  }, [isConnected, networkId, activeView]);
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {/* エラー表示 */}
          <ErrorAlert error={displayError} onClear={clearError} />
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-3">
                        <p className="text-sm text-blue-800">
                          Yoroiウォレットが接続されています。ネットワーク: {connectionInfo.networkName}
                        </p>
                      </div>
                    </div>
                    <div className="text-xs text-blue-600">
                      現在表示: {connectionInfo.activeViewLabel}
                    </div>
                  </div>
                </div>
              )}
