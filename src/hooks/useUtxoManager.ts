...
interface UseUtxoManagerReturn extends UtxoManagerState {
...
const parseUtxoCbor = async (cborUtxos: string[]): Promise<UTxO[]> => {
  ...
  console.log('📦 CSL loaded:', { 
    CSL: !!CSL, 
    TransactionUnspentOutput: !!CSL.TransactionUnspentOutput,
    available: Object.keys(CSL).filter(k => k.includes('Transaction')).slice(0, 5)
  });
...
export const useUtxoManager = (): UseUtxoManagerReturn => {
  ...
  const [state, setState] = useState<UtxoManagerState>({
    utxos: [],
    selectedUtxos: [],
    totalAda: BigInt(0),
    totalAssets: [],
  ...
  // Refresh UTxOs from wallet
  const refreshUtxos = useCallback(async (): Promise<void> => {
    console.log('🔄 refreshUtxos called:', { isConnected, api: !!api });
    ...
    console.log('📡 Starting UTxO fetch...');
    setState(prev => ({ ...prev, isLoading: true, error: null }));

    try {
      // Get UTxOs and change address in parallel
      console.log('📡 Calling api.getUtxos() and api.getChangeAddress()...');
      ...
      console.log('📦 Raw UTxO data:', { 
        ...
        changeAddr,
        cborUtxos: cborUtxos.slice(0, 2) // Show first 2 for debugging
      });

      // Parse CBOR UTxOs
      const parsedUtxos = await parseUtxoCbor(cborUtxos);
      console.log('✅ Parsed UTxOs:', { count: parsedUtxos.length, utxos: parsedUtxos });
      ...
      setState(prev => ({
        ...
        isLoading: false,
        error: null,
        // Clear selection if UTxOs changed
        selectedUtxos: [],
      }));
  ...
  // Auto-select UTxOs for a required amount (coin selection algorithm)
  const autoSelectForAmount = useCallback((requiredLovelace: bigint): boolean => {
    ...
    // Sort UTxOs by ADA amount (descending) for better selection
    ...
    if (selectedAmount >= requiredLovelace) {
      setState(prev => ({
        ...prev,
        selectedUtxos: [...prev.selectedUtxos, ...newSelection],
      }));
      return true;
    }

    return false; // Insufficient funds
  }, [state.utxos, state.selectedUtxos]);
  ...
  useEffect(() => {
    if (isConnected && api) {
      console.log('🔄 Starting UTxO refresh...');
      refreshUtxos().catch(error => {
        console.error('❌ UTxO refresh failed:', error);
      });
    } else {
      console.log('❌ Cannot refresh UTxOs - wallet not connected:', { isConnected, api: !!api });
    }
